...
const ProductsChart = () => {
    ...
    for (let index = 0; index < table.length; index++) {
        ...
        total = parseInt(total) + (parseInt(table[index].sellPrice ? table[index].sellPrice : 0) - parseInt(table[index].cost ? table[index].cost : 0))
        cost = parseInt(cost) + parseInt(table[index].cost ? table[index].cost : 0)
        sell = parseInt(sell) + parseInt(table[index].sellPrice ? table[index].sellPrice : 0)
        ...
        <div className='text-center'>
                ...
                <div className=''>
                    <table className="table">
                        ...
                        <tbody>
                            {
                                table.map(rowData => <tr className='text-center' key={rowData?._id}>
                                    <td>{rowData?.date}</td>
                                    ...
                                    <td>{rowData?.sQ ? (rowData?.cost / ((rowData?.cQ) - (rowData?.sQ))).toFixed(0) : 'X'}</td>
                                    <td className={ rowData?.sellPrice - rowData?.cost < 0 ? 'bg-danger text-light' : 'bg-primary text-light'}>{rowData?.sellPrice - rowData?.cost ? rowData?.sellPrice - rowData?.cost : 'X'}</td>
                                    <td>
                                        <Link className='me-1' to={`/update/${(rowData?._id)}`}><i className="fas fa-edit"></i></Link>
                                        <i onClick={()=>deleteBtn(rowData?._id)} className="ms-1 fas fa-trash"></i>
                                    </td>
                                </tr>)
                            }
                            <tr className='text-center'>
                                ...
                                <td colSpan={2} className={ total < 0 ? 'text-danger text-start' : 'text-primary text-start'}>{total}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
