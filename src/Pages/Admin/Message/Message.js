...
const Message = () => {
    ...
    const deleteMsg = id =>{
        fetch(`http://localhost:7000/message/${id}`,{
            method:'DELETE'
        })
        .then(res => res.json())
        .then(data => {
    ...
    return (
        <div>
            ...
            <h5 className='text-end'>Total message {messages?.length}</h5>
                {
                    messages?.map(message => <div key={message?._id} className='w-100 m-3'>
                        <div className="row border-bottom">
                            <div className="col-md-11 text-decoration-none">
                                <div className="row">
                                <div className="col-md-4">
                                    <strong className="me-auto">Message from *{message?.personName}*</strong>
                                </div>
                            ...
                            <div className="col-md-1 text-end">
                                <i onClick={()=>deleteMsg(message?._id)} className="fas fa-trash"></i>
                            </div>
                        </div>
                        <div className="w-100">
